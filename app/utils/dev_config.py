...
class DevConfigFileNotFoundError(Exception):
    ...
    def __init__(
        self, message="设备配置文件不存在，整个文件路径最好是英文的且绝对不能带有空格"
    ):
...
class DevConfigFileInvalidError(Exception):
    ...
    def __init__(
        self, message="设备配置文件格式错误，整个文件路径最好是英文的且绝对不能带有空格"
    ):
        self.message = message
        super().__init__(self.message)
...
def getYtConfigFromConfigFile() -> Dict[str, YTConfigDataStruct]:
    ...
    try:
        ...
        raise DevConfigFileInvalidError(
            f"设备配置文件格式错误或文件路径有误，确保文件格式为xlsx或xls及文件路径中不能含有空格\n{str(devConfigFilePath)}"
        )
    except FileNotFoundError:
        raise DevConfigFileNotFoundError(
            f"设备配置文件不存在，确保文件格式为xlsx或xls及文件路径中不能含有空格\n{str(devConfigFilePath)}"
        )
    ws = (
        wb.active
    )  # 获取活动表，如果用户改变了默认的激活表，那这里也是会出错的地方，到时候会报配置文件为空
    ...
    devChannelList = [
        cell.value for cell in ws[YTCTFEnum.devChannel.value]
    ]  # 设备通道列数据
    devPortList = [cell.value for cell in ws[YTCTFEnum.devPort.value]]  # 设备端口列数据
    devUserNameList = [
        cell.value for cell in ws[YTCTFEnum.devUserName.value]
    ]  # 设备用户名列数据
    devPasswordList = [
        cell.value for cell in ws[YTCTFEnum.devPassword.value]
    ]  # 设备密码列数据
    wb.close()  # 用完就关
    ytConfigList = {}

    devConfigFileContentInvalidText = (
        ""  # 个别行的配置有问题，统计到一块然后raise给上层
    )
    ...
    for index in range(len(ytList)):
        ...
        try:
            ...
            if is_empty(
                temp_ytConfig.yt
            ):  # 如果月台名称这个单元格是空的，一般说明用户删除了表格数据，但xlsx仍会保存这个单元格的数据为None，提前跳出下
                continue

            devTypeResult = getValidDevType(
                str(devTypeList[index])
            )  # 成功返回正确的英文设备类型，不然返回的None，这个判断用了两行
            temp_ytConfig.devType = devTypeResult if devTypeResult is not None else None

            temp_ytConfig.devIP = (
                str(devIPList[index]) if isValidIPv4(str(devIPList[index])) else None
            )
            # int方法不能接None，所以要提前判断一下，如果是None就让is_instance_variables_has_empty去处理
            temp_ytConfig.devChannel = (
                int(devChannelList[index])
                if str(devChannelList[index]).isnumeric()
                else None
            )
            temp_ytConfig.devPort = (
                int(devPortList[index]) if str(devPortList[index]).isnumeric() else None
            )
            temp_ytConfig.devUserName = str(devUserNameList[index])
            temp_ytConfig.devPassword = str(devPasswordList[index])
            if is_instance_variables_has_empty(
                temp_ytConfig
            ):  # 上面的数值判断，如果有错误的数值就会被设置为None，记录原数值后统一raise给上层
            ...
            IndexError
        ) as errorInfo:  # 某个单元格为空，但应该不会存在这个问题的，先抓了再说吧
            logger.error(f"好像是未知错误，原报错信息{errorInfo}")
            raise DevConfigFileHandleErrorException(
                f"未知错误，原报错信息\n{errorInfo}"
            )
    ...
    if ytConfigList:  # 判断列表是否为空
        ...
        raise DevConfigFileContentIsEmptyException(
            f"设备配置文件是空的，注意检查是否填错格式\n{devConfigFilePath}"
        )
