...
def getExcelDataTableWidgetData(
    filePath: str,
    shipid_CID: str = None,
    scantime_CID: str = None,
    ytname_CID: str = None,
    scantimeformat: str = None,
) -> List[ExcelDataTableWidgetItemDataStruct]:
    # 返回excel表格中指定列的数据
    ...
    logInfoText = (
        ...
        + shipID_CID
        + "列表头为:"
        + str(ws[shipID_CID][0].value)
        + "\n"
        + scanTime_CID
        + "列表头为:"
        + str(ws[scanTime_CID][0].value)
        + "\n"
        + ytName_CID
        + "列表头为:"
        + str(ws[ytName_CID][0].value)
    )
    ...
    for index in range(len(shipIDList)):
        ...
        try:
            temp_ItemData.shipID = (
                int(shipIDList[index]) if shipIDList[index] is not None else None
            )
            temp_ItemData.scanTime = (
                datetime.strptime(scanTimeList[index], scanTimeFormat)
                if scanTimeList[index] is not None
                else None
            )
    ...
    if not edtw_ItemDataList:  # 判断列表是否为空
        raise FileContentIsEmptyException(
            f"要处理的文件是空的，注意检查是否填错列号\n{filePath}"
        )
    else:
        return edtw_ItemDataList


def getSameYTCountTableWidgetData(
    devConfigGenerate: YtBindDevConfigGenerate,
    edtw_ItemDataList: List[ExcelDataTableWidgetItemDataStruct],
) -> List[SameYTCountTableWidgetItemDataStruct]:
    ...
    for yt, shipCount in sameYTCount:
        ...
        devconfig = devConfigGenerate.send(
            yt
        )  # 向生成器发送月台名称,生成器会返回设备配置
        temp_ItemDataList.devChannel = (
            f"{devconfig.devIP}-{str(devconfig.devChannel)}" if devconfig else "未配置"
        )
