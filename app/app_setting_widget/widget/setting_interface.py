...
from qfluentwidgets import FluentIcon as FIF, ScrollArea, PrimaryPushSettingCard, HyperlinkCard
from qfluentwidgets import (SettingCardGroup, SwitchSettingCard, ExpandLayout)

from app.app_setting_widget.widget.custom_setting_card import CustomComboBoxSettingCard, CustomOptionsSettingCard, CustomSpinBoxSettingCard, CustomCompactSpinBoxSettingCard
...
class SettingInterface(ScrollArea):
    def __init__(self, parent=None):
        ...
        self.updateOnStartUpCard = SwitchSettingCard(FIF.UPDATE, '在应用程序启动时检查更新', '新版本将更加稳定并拥有更多功能（建议启用此选项）', configItem=None, parent=self.updateSoftwareGroup)

        # 关于
        self.aboutGroup = SettingCardGroup("关于", self.scrollWidget)
        self.helpCard = HyperlinkCard(HELP_URL, "打开帮助页面", FIF.HELP, "教程", "发现新功能并了解有关 ATO 的使用技巧", self.aboutGroup)
        self.feedbackCard = PrimaryPushSettingCard('提供反馈', FIF.FEEDBACK, '提供反馈', "通过提供反馈帮助我们改进 ATO", self.aboutGroup)
        self.aboutCard = PrimaryPushSettingCard("检查更新", FIF.INFO, "关于", f"© 版权所有 {YEAR}, {AUTHOR}. 当前版本 {VERSION}", self.aboutGroup)
    ...
    def __initWidget(self):
        ...
        with open(f'{str(hlPath)}/resource/qss/{theme}/setting_interface.qss', encoding='utf-8') as f:
