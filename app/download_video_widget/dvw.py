...
from UnifyNetSDK.dahua.dh_playsdk_exception import (
    DHPlaySDKException,
    PLAY_NO_FRAME,
    PLAY_OPEN_FILE_ERROR,
)
...
from app.utils.tools import (
    findItemTextInTableWidgetRow,
    AlignCenterQTableWidgetItem,
    removeDir,
)
...
class DVWclass(BaseDVW):
    ...
    def __init__(self, parent=None):
        ...
        self.devConfigGenerate = (
            parent.devConfigGenerate
        )  # 因为加载设备配置生成器需要读取一个文件，所以由父类统一初始化并激活之后再下发给子类，更重要的是主窗体那边负责exception的报错弹窗
        ...
        self.classifyDownloadArgsByDevIP = (
            {}
        )  # 把下载参数按照ip分类，因为最终下载时是以ip为单位的
        self.classifyDownloadArgsIndexByFolder = (
            {}
        )  # 将classifyDownloadArgsByDevIP按照文件夹分类，方便跟本地的文件夹进行对比，以得到downloadErrorArgs
        self.downloadErrorArgs = (
            {}
        )  # 收集下载错误的下载参数，和classifyDownloadArgsByDevIP是相同结构的
        self.remkDirWithDownloadErrorArgs = (
            set()
        )  # 用于记录需要重新创建的文件夹，在文件下载完成之后的空档期用户是有可能删除所有文件夹的。1 “下载器没有权限创建文件夹”，2 遍历downloadErrorArgs取文件夹列表性能上不划算，几乎相当于下载器中每次做个文件夹是否存在的检查了。所以创建这个变量
        ...
        self.downloadResultList: ListProxy = (
            None  # 从manager拿的进程通信列表对象，它的方法定义在multiprocess.managers.BaseListProxy
        )
        self.downloadResultListCondition: ConditionProxy = (
            None  # 负责downloadResultList的进程安全同步和非空任务事件通知
        )
        ...
        self.ipRowIndexIndownloadProgress_TW = (
            {}
        )  # 这个变量是用来存储ip所在行的索引的，用来更新进度条的
        self.startDownloadThreadInstance = (
            None  # 负责开进程池的线程,startDownloadThread
        )
        self.totalDownloadCount = None  # 下载总量，用来更新 下载进度 标签
        self.downloadedCount = None  # 下载成功的数量。 　只能从那个线程中进行更改。也是用来更新 下载进度 标签
        self.downloadArgsNumInFolderDict = (
            {}
        )  # 从文件夹的角度统计下载参数的数量，也就是每个文件夹下应有文件数量.这是fileCount_TW要用的变量，取自下载前
        self.downloadStatusChange.connect(
            self.updateDownloadStatusUI
        )  # 这个信号也是getDownloadResultThread发出来的
        self.downloadStatusDone.connect(
            self.do_downloadStatusDone
        )  # 由于 下载线程是一个多线程，而下载结束后需要进行一些UI上的改动，故需要线程发送下载完成的信号，然后由槽函数负责更新UI
    ...
    def classifyArg(self, eflw_ItemDataList: List[ExcelFileListWidgetItemDataStruct]):
        ...
        self.downloadArgsNumInFolderDict = (
            {}
        )  # 数值初始化，从文件夹的角度统计下载参数的数量，也就是每个文件夹下应有文件数量
        for eflw_ItemData in eflw_ItemDataList:
            folderName = Path(eflw_ItemData.excelFilePath).stem
            Path(DVW_DOWNLOAD_VIDEO_PATH / folderName).mkdir(
                exist_ok=True
            )  # downloader貌似没能力创建文件夹
            self.downloadArgsNumInFolderDict[folderName] = len(
                eflw_ItemData.edtw_ItemDataList
            )  # 从文件夹的角度统计下载参数的数量，也就是每个文件夹下应有文件数量
            for edtw_ItemData in eflw_ItemData.edtw_ItemDataList:
                fileName = (
                    str(edtw_ItemData.shipID) + DVW_DOWNLOAD_FILE_SUFFIX
                )  # 单号加统一的文件后缀名mp4
                filePath = str(DVW_DOWNLOAD_VIDEO_PATH / folderName / fileName)
                scanTime = datetime.strptime(
                    str(edtw_ItemData.scanTime), DVW_DATETIME_FORMAT
                )  # 上层传来的时间数值是str类型的
                v_ytName = edtw_ItemData.ytName
                devConfig = self.devConfigGenerate.send(
                    v_ytName
                )  # 传月台名，拿对应的设备配置数据类实例
                if devConfig is None:  # 只有能拿到配置的月台才会进行后续处理
                    continue  # 且是必须存在的，所以未配置月台的筛选过程就放到dvw这边了
                devArgStruct = isDevIpExist = self.classifyDownloadArgsByDevIP.get(
                    devConfig.devIP, None
                )
                if isDevIpExist is None:  # 我存了两个变量名
                    devArgStruct = (
                        DevLoginAndDownloadArgStruct()
                    )  # 一个用来判断，一个用来指向数据结构
                    devArgStruct.devType = (
                        devConfig.devType
                    )  # 如果数据类实例不存在，就新建一个，然后指向这个数据类实例
                ...
                downloadArg = DownloadArg(
                    savePath=filePath,
                    downloadTime=scanTime,
                    ytName=v_ytName,
                    channel=v_channel,
                )  # 月台名和通道是冗余的存储，但不这么写的话，解析和存储的复杂度
                devArgStruct.downloadArgList.append(
                    downloadArg
                )  # 如果数据类实例存在，那就直接改呗
                self.classifyDownloadArgsByDevIP[devConfig.devIP] = (
                    devArgStruct  # 这里重新改一下字典的value，应该也不算什么大事，不然就得写重复代码了
                )

        self.classifyDownloadArgsIndexByFolder = (
            {}
        )  # # 将classifyDownloadArgsByDevIP按照文件夹分类，不复制下载参数，而是取下载参数在classifyDownloadArgsByDevIP中的索引
        for devIP, devArgStruct in self.classifyDownloadArgsByDevIP.items():
            for downloadArgIndex, downloadArg in enumerate(
                devArgStruct.downloadArgList
            ):
                savePath = Path(downloadArg.savePath)
                folderName = str(savePath.parent.stem)  # 获取文件夹名称
                fileName = str(savePath.stem)  # 获取文件名称
                dirDict = self.classifyDownloadArgsIndexByFolder.get(
                    folderName, {}
                )  # 尝试取出已有的 文件夹字典
                dirDict[fileName] = [
                    devIP,
                    downloadArgIndex,
                ]  # 这一步就已经把同文件夹下的相同单号给过滤掉了，但如果其他参数不相同就很头疼了，比如一个单号有两次三超的扫描时间记录
    ...
    def getDownloadResultThread(self):
        ...
        while True:
            with self.downloadResultListCondition:  # 查和改的操作都要在锁下进行。如果某个对列表的操作不在锁下，就不能保证是进程安全的了，python随时有可能把权限交出去
                ...
                self.downloadResultList[:] = (
                    []
                )  # ListProxy没有提供clear方法。它的方法定义在multiprocess.managers.BaseListProxy
            for (
                resultStruct
            ) in (
                result_List
            ):  # 这一步是极有可能存在速度瓶颈的，应该创一个独立缓冲区来保存下载结果，然后慢慢地让窗体更新数据。毕竟生产者(进程池)有8个，而消费者(窗体更新UI)只有一个。
                self.downloadStatusChange.emit(
                    resultStruct
                )  # 如果堵在这会很严重，不仅子进程下载结果发不过来，连下载进度都会被推迟。如果把阻塞都删掉呢？

    def updateDownloadStatusUI(self, resultList):
        """
        负责更新下载状态UI的槽函数。
        """
        widgetEnum, key_devIP, downloadArgList_index, downloadStatus = (
            resultList  # 子进程那边发来一个列表内含四个元素，依次为要操作的组件枚举，设备ip，下载参数索引，上报状态
        )
        try:  # 这样很聪明，避免了进程通信大数据的耗时传递
            devArgStruct = self.classifyDownloadArgsByDevIP.get(
                key_devIP
            )  # 按照ip拿DevLoginAndDownloadArgStruct
            downloadArg = devArgStruct.downloadArgList[
                downloadArgList_index
            ]  # 在按照索引取出其他需要显示的数据
        except KeyError:
        ...
        try:
            rowIndexIndownloadProgress_TW = self.ipRowIndexIndownloadProgress_TW[
                key_devIP
            ]  # 这个变量是用来存储ip所在行的索引的，用来更新进度条的
        except KeyError:
        ...
        if widgetEnum == DVWTableWidgetEnum.DOWNLOAD_STATUS_TABLE:
            itemTextList = [
                ...
                downloadArg.channel,
                downloadArg.ytName,
            ]  # 把所有要显示在tableWidget组件上的文本整合到列表中，一个for就能显示上去了
            savePath = Path(downloadArg.savePath)
            relative_path = savePath.relative_to(
                DVW_DOWNLOAD_VIDEO_PATH.parent
            )  # 缩短一下路径。 TODO 这里应该是可以用PROJECT_ROOT_PATH代替的
            itemTextList.extend(
                [relative_path, downloadArg.downloadTime, downloadStatus]
            )
            ...
            if downloadStatus == "下载成功":
                progressItemWidget = self.ui.downloadProgress_TW.cellWidget(
                    rowIndexIndownloadProgress_TW, 1
                )
                progressItemWidget.addOne()
                self.downloadedCount += 1
                self.ui.downloadStatus_TL.setText(
                    f"下载进度-({self.downloadedCount}/{self.totalDownloadCount})"
                )
    ...
    def startDownloadThread(self, downloadArgs):
                ...
                try:
                    ...
                    tasks.append(
                        executor.submit(
                            downloader,
                            self.downloadResultList,
                            self.downloadResultListCondition,
                            devArgStruct,
                        )
                    )
        ...
        self.downloadDone = (
            True  # 开启关闭标志，getDownloadResultThreadInstance线程可以关了
        )
    ...
    def handleDownloadList(
        self, eflw_ItemDataList: List[ExcelFileListWidgetItemDataStruct]
    ):
        # 点击开始下载按钮后的第一个执行的方法
    ...
    def beforeDownload(self, downloadArgs):
        ...
        for (
            devIP,
            devArgStruct,
        ) in downloadArgs.items():  # 设置ui，下载进度表格中插入进度条组件
            inIp_count = len(devArgStruct.downloadArgList)  # 每个ip单独的下载数量
            self.totalDownloadCount += inIp_count
            aItem = AlignCenterQTableWidgetItem(
                str(devIP)
            )  # 1整个下载流程中有很多变量可以进行且需要多次统一初始化处理
            self.ui.downloadProgress_TW.setItem(
                row, 0, aItem
            )  # 2可以把它们放到一个方法中
            self.ipRowIndexIndownloadProgress_TW[devIP] = (
                row  # 3这个UI的初始化处理也不应该放在这里，最好也放到一个方法里
            )
            ...
            row += 1  # 换行
        self.ui.downloadStatus_TL.setText(
            f"下载状态-(0/{self.totalDownloadCount})"
        )  # 设置ui，更新下载状态标签文本
        ...
        for (
            ...
            self.downloadArgsNumInFolderDict.items()
        ):  # 设置ui，填充 文件数量表格 中的 预下载数量列 中的数据
            row, isRowExist = findItemTextInTableWidgetRow(
                self.ui.fileCount_TW, folderName
            )  # 查找 对应行 是否已存在
            if not isRowExist:
                folderNameItem = AlignCenterQTableWidgetItem(
                    str(folderName)
                )  # 文件夹名称
        ...
        self.getDownloadResultThreadInstance = threading.Thread(
            target=self.getDownloadResultThread
        )  #
        self.getDownloadResultThreadInstance.start()
        self.startDownloadThreadInstance = threading.Thread(
            target=self.startDownloadThread,
            args=[
                downloadArgs,
            ],
        )  #
    ...
    def on_reDownload_PB_clicked(self):
        ...
        for (
            ...
            self.remkDirWithDownloadErrorArgs
        ):  # 先创建文件夹，主下载按钮的文件夹创建在classifyArg中，
            pathObject = Path(
                DVW_DOWNLOAD_VIDEO_PATH / folderName
            )  # 而重新下载前用户是有可能删除文件夹的，我这里稍微的确保一下文件夹的存在
            pathObject.mkdir(
                exist_ok=True
            )  # 我当然知道如果真的想保证文件夹路径存在就应该在下载器中实现这个功能
        self.beforeDownload(
            self.downloadErrorArgs
        )  # 可这样会浪费多少时间性能资源，下载器还是个多进程的，指不定整上个什么查不出来的bug
    ...
    def on_convert_PB_clicked(self):
        ...
        fileList = [
            pathObject
            for pathObject in Path(DVW_DOWNLOAD_VIDEO_PATH).rglob("*")
        ...
        for filePath in mp4FileList:
            ...
            try:
                ...
                InfoBar.error(
                    title="错误",
                    content=f"{relative_path}文件打开失败，已自动删除",
                    orient=Qt.Horizontal,
                    isClosable=True,
                    position=InfoBarPosition.TOP_LEFT,
                    duration=3000,
                    parent=self.ui.fileCount_TW,
                )
                logger.error(f"{filePath}的mp4文件打开失败，已自动删除")
            except PLAY_NO_FRAME:
                InfoBar.error(
                    title="错误",
                    content=f"{relative_path}文件没有有效帧，已自动删除",
                    orient=Qt.Horizontal,
                    isClosable=True,
                    position=InfoBarPosition.TOP_LEFT,
                    duration=3000,
                ...
                InfoBar.error(
                    title="错误",
                    content=f"{relative_path}文件转换失败，错误代码{type(e).__name__}，已自动删除",
                    orient=Qt.Horizontal,
                    isClosable=True,
                    position=InfoBarPosition.TOP_LEFT,
                    duration=3000,
                ...
                try:
                    ...
                    InfoBar.error(
                        title="错误",
                        content=f"{filePath}自动删除失败，错误代码{str(e)}",
                        orient=Qt.Horizontal,
                        isClosable=True,
                        position=InfoBarPosition.TOP_LEFT,
                        duration=3000,
    ...
    def on_deleteAllFile_PB_clicked(self):
        ...
        errorMsgBox = MessageBox(
            "确认", "是否要清空pic文件夹中的所有子级文件夹和文件？", self
        )
    ...
    def afterDownload(self):
        ...
        for first_level_dir in Path(DVW_DOWNLOAD_VIDEO_PATH).iterdir():
            ...
            for (
                second_level_file
            ) in first_level_dir.iterdir():  # 遍历日期目录下的文件夹
                if (
                    second_level_file.is_file()
                    and second_level_file.suffix == DVW_CONVERTED_FILE_SUFFIX
                ):  # 只有文件且后缀为.jpg才算下载成功（我会在统计前做一个mp4转jpg的操作，程序走到这还是存在mp4文件的话就说明这个mp4文件有问题（转换失败），那就的重新下载了）
                    folderName = first_level_dir.stem  # 文件夹名称
                    fileName = second_level_file.stem  # 文件名称
                    dirDict = localDirsDict.get(
                        folderName, {}
                    )  # 尝试取出已有的 文件夹字典
                    dirDict[fileName] = []
        ...
        for folderName, folderContent in localDirsDict.items():
            ...
            for fileName in list(
                ...
                if (
                    fileName in self.classifyDownloadArgsIndexByFolder[folderName]
                ):  # 检查文件名是否在该文件夹的分类下载参数索引中
                    del self.classifyDownloadArgsIndexByFolder[folderName][
                        fileName
                    ]  # 如果是，则从索引中删除该文件名
        # 移除所有内容为空的文件夹
        self.classifyDownloadArgsIndexByFolder = {
            folder: content
            for folder, content in self.classifyDownloadArgsIndexByFolder.items()
            if content
        }
        ...
        for folderName, folderContent in self.classifyDownloadArgsIndexByFolder.items():
            self.remkDirWithDownloadErrorArgs.add(
                folderName
            )  # 这里不用集合也行，因为上面的folderName已经用字典的key特性去重过了
            for fileName, fileContent in folderContent.items():
                ...
                try:
                    ...
                    logger.error(
                        f"{devIP}的DevLoginAndDownloadArgStruct不存在,这是不应该发生的，说明程序有BUG，很严重！"
                    )
                    continue
                downloadArg = devArgStruct.downloadArgList[
                    downloadArgList_index
                ...
                if error_DevArgStruct is None:
                    error_DevArgStruct = (
                        DevLoginAndDownloadArgStruct()
                    )  # 如果数据类实例不存在，就新建一个数据类实例
                    error_DevArgStruct.copy_from(
                        devArgStruct, keepDownloadArgListEmpty=True
                    )
        ...
        for devIP, devArgStruct in self.downloadErrorArgs.items():
            ...
            ipItem = AlignCenterQTableWidgetItem(
                str(devIP)
            )  # 将ip和错误数量写入表格组件
        ...
        InfoBar.warning(
            ...
            position=InfoBarPosition.TOP_LEFT,
            duration=1000,
            parent=self.ui.downloadError_TW,
        )
