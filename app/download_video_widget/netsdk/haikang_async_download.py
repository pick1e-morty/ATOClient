        ...
        while True:
            ...
            for downloadHandle in list(
                ...
                try:
                    ...
                    HKNetSDKException
                ) as e:  # 除非把stopDownLoadTimer拆开，不然拿不到具体的信息，这块应该没什么大问题
                    logger.error(
                        f"{downloadHandle}的stopDownLoadTimer执行失败，错误代码{str(type(e).__name__)}"
                    )
                savePath, iNDEX, TimeoutNum = self.downloadHandleDict[downloadHandle]
                if downloadPos == 100:
                    try:
                        tsPic(
                            absVideoPath=savePath
                        )  # 这个也需要上报状态，先不写，因为整体使用逻辑还有点不成熟
                    ...
                    if TimeoutNum == 0:
                        ...
                        TimeoutNum -= 1
                        with self.downloadHandleDictCondition:
                            self.downloadHandleDict[downloadHandle] = [
                                savePath,
                                iNDEX,
                                TimeoutNum,
                            ]
                        logger.debug(
                            f"下载ID {downloadHandle}，文件地址 {savePath}，下载进度 {downloadPos}，剩余超时次数 {TimeoutNum}"
                        )
            sleep(0.5)


def HaikangDownloader(
    downloadResultList,
    ...
    def updateDownloadStatusFun(
        ...
        logger.debug(
            f"{widgetEnum}更新状态：{deviceAddress} {f_iNDEX} {f_status}"
        )  # TODO UI那边又出现上报数量不对等的情况，不知道是关闭句柄线程的问题还是什么
    ...
    execute_operation(
        haikangClient.logopen, [str(absLogPath)], "打开日志成功", "打开日志失败"
    )
    ...
    try:
        ...
        updateDownloadStatusFun(
            0, sucessText, widgetEnum=DVWTableWidgetEnum.DOWNLOAD_PROGRESS_TABLE
        )
        ...
        updateDownloadStatusFun(
            0, errorStr, widgetEnum=DVWTableWidgetEnum.DOWNLOAD_PROGRESS_TABLE
        )
    ...
    logger.trace(
        "登录成功的测试，打印硬盘数量", device_info.struDeviceV30.byDiskNum
    ...
    for iNDEX, downloadArg in enumerate(devArgs.downloadArgList):
        ...
        try:
            downLoadHandle = haikangClient.asyncDownLoadByTime(
                userID, downloadbytimeArg
            )
            if downLoadHandle != -1:
                with downloadHandleDictCondition:
                    downloadHandleDict[downLoadHandle] = [
                        downloadArg.savePath,
                        iNDEX,
                        TimeoutNum,
                    ]
...
if __name__ == "__main__":
    ...
    HaikangDownloader(
        t_downloadResultList, t_downloadResultListCondition, testUserConfig
    )
