...
def TimeDownLoadPosCallBack(
    lPlayHandle, dwTotalSize, dwDownLoadSize, index, recordfileinfo, dwUser
):
...
class DahuaDownloader(object):
    ...
    def updateDownloadStatusFun(
        ...
        """
        self.downloadResultList.append(
            [widgetEnum, self.deviceAddress, f_iNDEX, f_status]
        )
    ...
    def login(self):
        ...
        if self.loginID == 0:
            ...
            self.updateDownloadStatusFun(
                0, errorMsg, DVWTableWidgetEnum.DOWNLOAD_PROGRESS_TABLE
            )
            sys.exit(1)
        else:
            sucessMsg = "登录成功"
            ...
            self.updateDownloadStatusFun(
                0, sucessMsg, DVWTableWidgetEnum.DOWNLOAD_PROGRESS_TABLE
            )
    ...
    def startDownload(self):
        ...
        for iNDEX, downloadArg in enumerate(self.devArgs.downloadArgList):
            ...
            downloadID = self.sdk.DownloadByTimeEx(
                self.loginID,
                nchannel,
            ...
            if not downloadID:
                ...
                self.handle_index[downloadID] = (
                    iNDEX  # 这个是冗余的，是为了获取最大时效性。
                )
