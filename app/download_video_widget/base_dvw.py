...
class BaseDVW(QWidget):
    ...
    def doBigDogShouldDo(self, pathChanged):
        ...
        deletePaths = [str(pathObject) for pathObject in list(self.subDirs - curSubDir)]  # 要删除的路径
        addPaths = [str(pathObject) for pathObject in list(curSubDir - self.subDirs)]  # 要添加的路径
        if deletePaths:
            self.smallDog.removePaths(deletePaths)  # 如果欲删除路径不为空，则从小狗中删除该路径
            for pathObject in deletePaths:
                dirName = Path(pathObject).stem  # 取路径最后一级
                findedItems = self.ui.fileCount_TW.findItems(dirName, Qt.MatchExactly)  # 先查到这个这个文件夹所在表格中的行
                if findedItems:
        ...
        if addPaths:
            self.smallDog.addPaths(addPaths)  # 如果欲添加的路径不为空，则全部加到小狗中，并逐个激活小狗的槽函数
    ...
    def doSmallDogShouldDo(self, pathChanged):
        if not Path(pathChanged).exists():  # QFileSystemWatcher不支持反应文件夹在被修改时的状态
            logger.trace("如果某个文件夹被删除了，本槽函数还是会被先于大狗的槽函数激活，所以作此判断。路径不存在的删除由大狗负责")
            return  # 如果某个文件夹被删除了，本槽函数还是会被先于大狗的槽函数激活，所以作此判断。路径不存在的删除由大狗负责
        fileList = [pathObject for pathObject in Path(pathChanged).iterdir() if pathObject.is_file()]  # 遍历激活路径
        mp4FileList = [pathObject for pathObject in fileList if pathObject.suffix == DVW_DOWNLOAD_FILE_SUFFIX]  # 取mp4后缀文件路径
        jpgFileList = [pathObject for pathObject in fileList if pathObject.suffix == DVW_CONVERTED_FILE_SUFFIX]  # 取jpg后缀文件路径
        dirName = Path(pathChanged).stem  # 取路径最后一级
        jpgFileCount = len(jpgFileList)
        mp4FileCount = len(mp4FileList)
        row, isRowExist = findItemTextInTableWidgetRow(self.ui.fileCount_TW, dirName)  # 查找 对应行 是否已存在
        if not isRowExist:
