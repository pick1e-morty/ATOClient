...
from app.utils.dev_config import (
    ...
    DevConfigFileNotFoundError,
    DevConfigFileInvalidError,
    DevConfigFileContentIsEmptyException,
    DevConfigFileContentIsInvalidException,
    DevConfigFileHandleErrorException,
)
from app.utils.forms_config import (
    getFormsConfigDict,
    VdtStrNotUpperError,
    VdtDateTimeFormatError,
    FormsConfigWrite,
)
...
class MainWindow(BaseMainWindow):
    ...
    def checkUpdate(self):
        ...
        localVersion, _suffixIndex = QVersionNumber.fromString(
            self.versionManager.currentVersion[1:]
        )
        ignoreVersion, _suffixIndex = QVersionNumber.fromString(
            self.versionManager.ignoreVersion[1:]
        )
        ...
        if remoteVersion == ignoreVersion:
            ...
            newVersion, releaseTitle, releaseInfo = (
                self.versionManager.getUpdateReleaseInfo()
            )
            updateMsgBox = UpdateInfoMsgBox(releaseTitle, releaseInfo, self)
            updateMsgBox.accepted.connect(
                lambda: QDesktopServices.openUrl(QUrl(RELEASE_URL))
            )  # 跳转到更新页面
            updateMsgBox.ignoreSignal.connect(
                lambda: self.write_ignore_version2Configini(newVersion)
            )  # 写入忽略版本号到配置文件
            updateMsgBox.raise_()
            updateMsgBox.exec()
    ...
    def load_devConfigGenerate(self):
        ...
        try:
            ...
            )  # 启动生成器,这步将执行getYtConfigFromConfigFile函数，异常也是从这个函数raise出来的
        except (
            DevConfigFileNotFoundError,
            DevConfigFileInvalidError,
            DevConfigFileContentIsEmptyException,
            DevConfigFileHandleErrorException,
            DevConfigFileContentIsInvalidException,
        ) as errorText:  # 设备配置文件路径是写的相对的，写在处理函数中的，可能会出现点问题吧
    ...
    def initUI(self):
        ...
        self.addSubInterface(
            self.aswInterface, FIF.SETTING, "设置", NavigationItemPosition.BOTTOM
        )
    ...
    def connectWidgetSignal(self):
        # 连接组件信号
        self.dvwInterface.ui.startDownLoad_PB.clicked.connect(
            self.dvw_startDownLoad_PB_clicked
        )
    ...
    def do_afterSwitchFun(self, InterFaceWidget):
        ...
        if excelFile_LW_CurrentItem is not None:
            self.epwInterface.saveTableDataToListWidgetItemData(
                excelFile_LW_CurrentItem
            )
    ...
    def dvw_startDownLoad_PB_clicked(self):
        ...
        for i in range(self.epwInterface.ui.excelFile_LW.count()):
            excelFile_LW_ItemData = self.epwInterface.ui.excelFile_LW.item(i).data(
                Qt.UserRole
            )
